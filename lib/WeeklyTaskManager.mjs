...
class WeeklyTaskManager {
    ...
    async _startTask() {
        ...
        await this.reportManager.sayHi(process.env.NOTICE_CHANNEL);

        const todayAfternoon = getMomentNow().set('h', config.urge_start_at).set('m', 0).set('s', 0);
    ...
    async _urge() {
        ...
        await this.reportManager.sendAllMembersStatuses(process.env.NOTICE_CHANNEL);
